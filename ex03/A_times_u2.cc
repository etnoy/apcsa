...
#pragma omp parallel shared(p)
	...
	reset_vector(v1, N);
	time = walltime(&time_start);
	A_triangular_mult_ordinary(N, A, u, v1);   
	time = walltime(&time);
	...
	reset_vector(v1, N);
	time = walltime(&time_start);
	A_triangular_mult_optimized(N, A, u, v1);   
	time = walltime(&time);
	equality_test(N, v1, v_triag);
	cout << time << " sec " << endl;
	...
	reset_vector(v1, N);
	time = walltime(&time_start);
	A_triangular_mult_self_opt(N, A, u, v1);   
	time = walltime(&time);
	equality_test(N, v1, v_triag);
	cout << time << " sec " << endl;
...
#pragma omp parallel for
	for(int i = 0; i<N/2+1; i++)
	{
		...
		for(int j = i; j<N; j++)
			temp_sum1 += A[ i*N + j ]*u[j];
		v[i] = temp_sum1;

		for(int j = N-i; j<N; j++)
			temp_sum2 += A[ (N-i)*N + j ]*u[j];
		v[N-i] = temp_sum2;
