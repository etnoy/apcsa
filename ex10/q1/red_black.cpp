...
{  
	*local_res =0.0;
	...
	for(int i=1; i<N-1; i++)
		for(int j=1; j<N-1; j++)
			*local_res +=  pow(((u[i+1][j]+u[i-1][j]+u[i][j+1]+u[i][j-1]-4*u[i][j])/h -1.0),2.0);
...
{  
	...
	if(left!=MPI_PROC_NULL)
	{
		...
		MPI_Sendrecv(buff_send, n, MPI_DOUBLE, left, 0, 
				buff_recv, n, MPI_DOUBLE, left,0, MPI_COMM_WORLD,&status); 
		for (i=1; i < N-1; i++)
			u[i][1]=buff_recv[i-1];	
	}
	if(right!=MPI_PROC_NULL)
	{
		...
		MPI_Sendrecv(buff_send, n, MPI_DOUBLE, right, 0, 
				buff_recv, n, MPI_DOUBLE, right, 0, MPI_COMM_WORLD,&status); 
		for (i=1; i < N-1; i++)
			u[i][n]=buff_recv[i-1];	
	}
	if(top!=MPI_PROC_NULL)
	{
		for (i=1; i < N-1; i++)
			buff_send[i-1]=u[1][i];	
		MPI_Sendrecv(buff_send, n, MPI_DOUBLE, top, 0, 
				buff_recv, n, MPI_DOUBLE, top, 0, MPI_COMM_WORLD,&status); 
		for (i=1; i < N-1; i++)
			u[1][i]=buff_recv[i-1];	
	}
	if(bottom!=MPI_PROC_NULL)
	{
		for (i=1; i < N-1; i++)
			buff_send[i-1]=u[n][i];	
		MPI_Sendrecv(buff_send, n, MPI_DOUBLE, bottom, 0, 
				buff_recv, n, MPI_DOUBLE, bottom, 0, MPI_COMM_WORLD,&status); 
		for (i=1; i < N-1; i++)
			u[n][i]=buff_recv[i-1];	
	}
...
void red_black_GS(int N, double h, double **u)
{
	...
	for(i=1;i<N-1;i++)
	{
		for(j=i%2+1;j<N-1;j=j+2)
		{
			u[i][j]=(u[i+1][j]+u[i-1][j]+u[i][j+1]+u[i][j-1]-h)/4.0;
		}
	}
	//black nodes
	for(i=1;i<N-1;i++)
	{
		for(j=(i+1)%2+1;j<N-1;j=j+2)
		{
			u[i][j]=(u[i+1][j]+u[i-1][j]+u[i][j+1]+u[i][j-1]-h)/4.0;
		}
	}
